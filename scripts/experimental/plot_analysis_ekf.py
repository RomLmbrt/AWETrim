...
from awes_ekf.plotting import (
    plot_aerodynamics,
    plot_kinematics,
    plot_tether,
    plot_wind_velocity,
    plot_ekf_performance,
    personalized_plot,
    plot_system_performance,
)
from awes_ekf.plotting.plot_aerodynamics import (
    plot_turn_rate_identification,
)
...
def select_plot_category():
    ...
    categories = {
        ...
        "8": (
            "Turn-rate law identification (per-phase fit)",
            plot_turn_rate_identification,
        ),
    }
...
def get_time_mask(data: pd.DataFrame) -> pd.Series:
    ...
    start_time = float(
        input(
            f"Enter the start time for plotting (seconds, or leave empty for {min_time:.2f}): "
        )
        or min_time
    )
    end_time = float(
        input(
            f"Enter the end time for plotting (seconds, or leave empty for {max_time:.2f}): "
        )
        or max_time
    )
...
def main():
    ...
    while True:
        ...
        if plot_function:
            plot_function(
                filtered_ekf_output_df, filtered_flight_data_df, config_data
            )  # Call the selected function with filtered data

        # Ask the user if they want to plot another category
        another_plot = (
            input("Would you like to plot another category? (yes/no): ").strip().lower()
        )
        if another_plot != "yes":
