...
def build_actuation_case_folder(config: dict) -> str:
    ...
    depower_tag = format_length_tag(
        config.get("power_tape_final_extension", CONFIG_DEFAULTS["power_tape_final_extension"])
    )
...
def resolve_initial_geometry_rotation_kwargs(config: dict) -> dict:
    ...
    if angle_deg is None and angle_rad is None:
        angle_deg = [
            0.0,
            float(config.get("initial_geometry_rotation_deg", CONFIG_DEFAULTS["initial_geometry_rotation_deg"])),
            0.0,
        ]
    return {
        "angle_deg": angle_deg,
        "angle_rad": angle_rad,
        "point": config.get("initial_geometry_rotation_point", CONFIG_DEFAULTS["initial_geometry_rotation_point"]),
        "axes": config.get("initial_geometry_rotation_axes", CONFIG_DEFAULTS["initial_geometry_rotation_axes"]),
    }
...
def configure_system_model_from_config(system_model: SystemModel, config: dict) -> None:
    ...
    system_model.timeder_speed_tangential = float(
        config.get("timeder_speed_tangential", CONFIG_DEFAULTS["timeder_speed_tangential"])
    )
    ...
    try:
        ...
        if "rest_lengths" in metadata:
            ...
            if rest_lengths_loaded.shape == np.asarray(l0_arr_default).shape:
                logging.info(f"Recovered rest_lengths from previous run: {h5_path.parent.name}")
