...
class WinchControllerCharacteristics:
    def __init__(self):
        self.v_max = 10.0
        self.v_m_list = np.linspace(-3.0, self.v_max, 1000)
    ...
    def get_force_controller_setpoint(self, v_m, v_cmd, p_gain_v, f_min, f_max, f_knee=None, force_slope_factor=None):
        if not f_knee:
            f_knee = f_max
        ...
        # print(v_knee)
        error = (v_cmd - v_m)
        output = p_gain_v * error  # This term simulates the normal pid controller (i-gain=d-gain=0).

        if v_m > v_knee:
            output = (f_max - f_knee) * np.tanh((v_m - v_knee) * force_slope_factor) + f_knee

        return self.cap_output(output, f_min, f_max)

    def get_force_controller_setpoint_function(self, v_cmd, p_gain_v, f_min, f_max, p_gain_f, f_knee, force_slope_factor):
        return [self.get_force_controller_setpoint(v_m=v_m,
                                                   v_cmd=v_cmd,
                                                   p_gain_v=p_gain_v,
                                                   f_min=f_min,
                                                   f_max=f_max,
                                                   f_knee=f_knee,
                                                   force_slope_factor=force_slope_factor) for v_m in self.v_m_list]

    def get_effective_controller_function(self, v_cmd, p_gain_v, f_min, f_max, p_gain_f, f_knee, force_slope_factor):
        ...
        return [self.get_force_controller_setpoint(v_m=v_m,
                                                   ...
                                                   f_max=f_max,
                                                   f_knee=f_knee,
                                                   force_slope_factor=force_slope_factor) / conversion_to_actual_force for v_m in self.v_m_list]

    def get_controller_setpoint_linear(self, v_m, v_cmd, p_gain_v, f_min, f_max):
        output = p_gain_v*(v_cmd-v_m)
        return self.cap_output(output, f_min, f_max)

    def get_effective_controller_function_linear(self, v_cmd, p_gain_v, f_min, f_max, p_gain_f):
        conversion_to_actual_force = 1.0 + 1.0 / (p_gain_f * self.torque_to_force)
        return [self.get_controller_setpoint_linear(v_m=v_m,
                                                    v_cmd=v_cmd,
                                                    p_gain_v=p_gain_v,
                                                    f_min=f_min,
                                                    f_max=f_max) / conversion_to_actual_force for v_m in self.v_m_list]
    ...
    def plot_controller_characteristics(self, controller_parameters: list):
        ...
        for power in power_values:
            plt.plot(self.v_m_list, self.get_power_isolines(self.v_m_list, power),
                     color='black',
                     alpha=0.5,
                     linewidth=0.5)
            x_value_text = 0
            if power < 0:
                ...
                x_value_text = self.v_max
            plt.text(x_value_text, self.get_power_isolines(x_value_text, power),
                     f'{power} kW',
                     color='black',
                     fontsize=10,
                     ha='right')
        ...
        for params in controller_parameters:
            controller_linear = self.get_effective_controller_function_linear(v_cmd=params.v_cmd,
        ...
        for params in controller_parameters:
            self.controller = self.get_effective_controller_function(v_cmd=params.v_cmd,
                                                                p_gain_v=params.p_gain_v,
                                                                p_gain_f=params.p_gain_f,
                                                                f_min=params.f_min,
                                                                f_max=params.f_max,
            ...
            plt.plot(self.v_m_list, self.controller,
    ...
    def plot_force_controller_setpoint(self, controller_parameters: list):
        ...
        for params in controller_parameters:
            controller = self.get_force_controller_setpoint_function(v_cmd=params.v_cmd,
                                                                     p_gain_v=params.p_gain_v,
                                                                     p_gain_f=params.p_gain_f,
                                                                     f_min=params.f_min,
                                                                     f_max=params.f_max,
                                                                     f_knee=params.force_knee,
                                                                     force_slope_factor=params.force_slope_factor)
            plt.plot(self.v_m_list, controller,
                     label=f'{params.plot_comment}: f_knee = {params.force_knee}, b={params.force_slope_factor}, '
                           f'f_min={params.f_min}, f_max={params.f_max}, Kp_v={params.p_gain_v}, Kp_f={params.p_gain_f},'
                           f' v_cmd={params.v_cmd}')
...
def main():
    parameters = [
        WinchControllerParameters(f_max=3500.0,
                                  f_min=440.0,
                                  force_knee=2500,
