...
from picawe.kinematics.my_data_processing import DataProcessing  # Your refactored DataProcessing class
...
class Fitting(DataProcessing):
    ...
    def __init__(self, file_path_full, file_path_cycle, file_path_waypoints,
                 cyc_idx=0, segment="RI", n_ctrl_pts=8):
    ...
    def FitLissajous(self):
        ...
        def residual(x):
            ...
            return np.concatenate((self.az_data - az_model, self.el_data - el_model)).ravel()
    ...
    def plot_fit(self, title_prefix="", ax=None, show_control_points=True):
        ...
        if show_control_points:
            axes[0].scatter(self.u_vals[self.fitted_indices_az], C_az_full,
        ...
        if show_control_points:
            axes[1].scatter(self.u_vals[self.fitted_indices_el], C_el_full,
                            c='red', s=25, edgecolors='black', label='Control Points', zorder=5)
