...
from picawe.kinematics.Other_code_by_theophile_dubois.my_RI_RO_data_processing import RI_RO_data_processing
import numpy as np
from scipy.optimize import least_squares
import matplotlib.pyplot as plt

class RI_RO_fitting(RI_RO_data_processing, Bspline_build):
    def __init__(self, file_path_full=None, file_path_cycle=None, file_path_waypoint=None, cyc_idx=0, p=3, n_ctrl=6,
                 c_penalty=1.0, v_penalty=0.0, eps_knot=1e-3):
        super().__init__(file_path_full, file_path_cycle, file_path_waypoint, cyc_idx)
        ...
        self.fit_RI_RO("spherical")
        self.fit_RI_RO("cartesian")

    def _compute_u_vals(self):
        self.S_sph = np.array(self.RI_RO_sph).T
        self.S_cart = np.array(self.RI_RO_cart).T
    ...
    def fit_RI_RO(self, mode="spherical"):
        # Implement the fitting procedure for the RI to RO transition

        if mode == "spherical":
            self.p0 = self.RI_RO_p0_sph
            self.pf = self.RI_RO_pf_sph
            self.S_data = self.S_sph# shape (N, 2)
            self.dim = 2
        elif mode == "cartesian":
            self.p0 = self.RI_RO_p0_cart
            self.pf = self.RI_RO_pf_cart
        ...
        S, dS = builder.evaluate_spline(spline_func, C0, U0)
        ...
        def residuals(params):
            ...
            S_fit, dS_fit = builder.evaluate_spline(spline_func, C, U)
            ...
            if mode == "spherical":
                ...
                res_extra = np.array(
                    [self.RI_RO_crs0 - avg_start, self.RI_RO_crsf - avg_end]
                ) * self.c_penalty
            else:
                ...
                res_extra = np.ravel(
                    np.array([vel_start - self.RI_RO_v0, vel_end - self.RI_RO_vf])
                ) * self.v_penalty
    ...
    def plot_fitted_path(self, mode="cartesian"):
        ...
        S_fit, _ = builder.evaluate_spline(spline_func)
        ...
        if mode == "spherical":
            ...
            ax_az.plot(np.linspace(0, len(self.az_cyc)/self.RI_RO_idxf, len(self.az_cyc)), self.az_cyc, label="Azimuth (data)", color="C0")
            ax_az.plot(self.u_vals, S_fit[:,0], "--", label="Azimuth (spline)", color="C1")
            ax_el.plot(np.linspace(0, len(self.az_cyc)/self.RI_RO_idxf, len(self.az_cyc)), self.el_cyc, label="Elevation (data)", color="C0")
            ax_el.plot(self.u_vals, S_fit[:,1], "--", label="Elevation (spline)", color="C1")
            ...
            # # ri_ro start/end
            ax_az.scatter(self.u_vals[0], self.p0[0], color="green", s=30, label="ri start")
            ax_az.scatter(self.u_vals[self.RI_RO_idxf], self.pf[0], color="red", s=30, label="ri end")
            ax_el.scatter(self.u_vals[0], self.p0[1], color="green", s=30, label="ri start")
            ax_el.scatter(self.u_vals[self.RI_RO_idxf], self.pf[1], color="red", s=30, label="ri end")
...
if __name__ == "__main__":
    ...
    obj = RI_RO_fitting(file_path_full=full_path, file_path_cycle=cycle_path, file_path_waypoint=waypoint_path, cyc_idx=0, p=3, n_ctrl=6,
