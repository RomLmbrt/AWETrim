...
from picawe.kinematics.Other_code_by_theophile_dubois.my_RI_data_processing import RI_data_processing
from picawe.kinematics.Other_code_by_theophile_dubois.my_RI_RO_data_processing import RI_RO_data_processing
from picawe.kinematics.Other_code_by_theophile_dubois.my_RO_RI_data_processing import RO_RI_data_processing
from picawe.kinematics.Other_code_by_theophile_dubois.my_Lisajous_fitting import Lisajous_fitting


class Fitting(build):
    """Performs spline fitting on azimuth and elevation data."""

    def __init__(self, r0, r1, az_data, el_data, u_vals, n_ctrl_pts=8):
        # Parameters
        self.r0 = r0
        self.r1 = r1
...
if __name__ == "__main__":
    ...
    fit_RI = Fitting(RI.RI_r0, RI.RI_r1, RI.RI_az, RI.RI_el, RI.u_vals, n_ctrl_pts=15)
    fit_RI.save_data("RI")

    # --- Reel In to Reel Out ---
    print("\n🔹 Fitting RI_RO segment...")
    RIRO = RI_RO_data_processing(full_path, cycle_path, waypoint_path, cyc_idx=0)
    fit_RIRO = Fitting(RIRO.RI_RO_r0, RIRO.RI_RO_r1, RIRO.RI_RO_az, RIRO.RI_RO_el, RIRO.u_vals)
    fit_RIRO.save_data("RI_RO")

    # --- Reel Out to Reel In ---
    print("\n🔹 Fitting RO_RI segment...")
    RORI = RO_RI_data_processing(full_path, cycle_path, waypoint_path, cyc_idx=0)
    fit_RORI = Fitting(RORI.RO_RI_r0, RORI.RO_RI_r1, RORI.RO_RI_az, RORI.RO_RI_el, RORI.u_vals)
